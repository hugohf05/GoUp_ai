...
indexes_to_create = [
    ("idx_descans_atleta_date", "CREATE INDEX idx_descans_atleta_date ON core_descans (atleta_id, CAST(data_inici AS date));"),
    ("idx_se_data_opt", "CREATE INDEX idx_se_data_opt ON core_sessioentrenament (data, atleta_id, ubicacio_id);"),
    ("idx_atleta_estat_dni", "CREATE INDEX idx_atleta_estat_dni ON core_atleta (estat, dni_atleta);"),
    ("idx_ubicacio_tipus_id", "CREATE INDEX idx_ubicacio_tipus_id ON core_ubicacio (tipus_ubicacio, id);"),
...
with connection.cursor() as cursor:
    ...
    print("\nRunning single-threaded query WITH indexes...")
    try:
        start = time.time()
        ...
        # Print the first few lines of the explain plan
        for r in rows[:40]:
            print(r[0])
    except Exception as e:

...
with connection.cursor() as cursor:
    ...
    print(f"Time without indexes: {time_without:.4f} seconds.")
    ...
    start = time.time()
    cursor.execute(f"EXPLAIN ANALYZE {query}")
    explain_with = cursor.fetchall()
    time_with = time.time() - start
    print(f"Time WITH indexes: {time_with:.4f} seconds.")
    
    for r in explain_with[:40]:
        print(r[0])

...
class Command(BaseCommand):
    ...
    def generate_registres_descans_informes(self, dnis):
        ...
        for i in range(1, NUM_REGISTRES + 1):
            ...
            h = time(hour=random.randint(22, 23), minute=random.randint(0, 59))
            d_inici_descans = timezone.make_aware(datetime.combine(data_reg - timedelta(days=1), h))
            ...
            reg = RegistreDiari(
                # Django no sap l'ID fins que guardem per bulk_create normal, però podem forçar un ID seqüencial si volem
                # O per ser OneToOne (informe necessita aquest id), en bulk_create podem tenir problemes.
                # Per tant generem un ID i l'usem explícitament.
                id=i,
                atleta_id=dni,
                data=data_reg,
                adaptabilitat_alimentacio=round(random.uniform(0.0, 100.0), 2),
                comentaris_entrenaments=fake.sentence() if random.random() > 0.5 else None,
                ...
                sensacions_generals=fake.sentence() if random.random() > 0.5 else None
            )
            reg_batch.append(reg)
            
            inf = InformeIA(
                registre_diari_id=i, # L'ID corresponent
                recomanacio_entrenament="Recomanació: " + fake.sentence(),
                recomanacio_alimentacio="Recomanació: " + fake.sentence(),
                recomanacio_descans="Recomanació: " + fake.sentence()
            )
            ...
            if i % BATCH_SIZE == 0 or i == NUM_REGISTRES:
                ...
                desc_batch, reg_batch, inf_batch = [], [], []
                self.print_progress(i, NUM_REGISTRES, "Registres i Informes")
